...
class Delete(unittest.TestCase):
  def test_insert(self):
    ...
    query = SQpy.ast.delete_from( 'cities', where=SQpy.ast.op_and( SQpy.ast.op_equal(SQpy.ast.identifier('country'), 'France'), SQpy.ast.op_inferior( SQpy.ast.identifier('latitude'), 3)))
    db.execute(query)

    self.assertEqual(len(db.dump_table('cities')), 3)
    self.assertEqual(db.dump_table('cities')[0]._asdict(), {'name': 'Linkoping', 'population': 152966, 'longitude': 58.410833, 'latitude': 15.621389, 'country': 'Sweden', 'comment': 'My home town'})
    self.assertEqual(db.dump_table('cities')[1]._asdict(), {'name': 'Strasbourg', 'population': 768868, 'longitude': 48.58, 'latitude': 7.75, 'country': 'France', 'comment': None})
    self.assertEqual(db.dump_table('cities')[2]._asdict(), {'name': 'London', 'population': 8538689, 'longitude': 51.507222, 'latitude': -0.1275, 'country': 'United Kingdom', 'comment': None})
