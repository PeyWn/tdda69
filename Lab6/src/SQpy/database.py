...
class database(object):
    ...
    def execute(self, query):
        if query.token == token.create_table:
            ...
            try:
                ...
                for i in range(len(query.columns)):
                    entry = entry._replace(
                        **{query.columns[i]: query.values[i]})
            ...
            if isinstance(query.columns, ast) and query.columns.token == token.star:
                ...
                try:
                    entries = self.ast_op(
                        query.where, self.e_list[query.from_table])
                except:
        ...
        elif query.token == token.inner_join:
            print('Hello! ', self)
    ...
    def ast_op(self, node, entry_list):
        if node.token == token.op_and:
            ...
            #print('====== Start od debug ======')
            # print(oper)
            #print('======= End of Debug ======')

            if oper[1] != []:
                return oper[0][0]/oper[1][0]
          ...
          for e in entry_list:
              for id in node.operands[0].identifier:
                  print(id)
                  print(e._asdict()[id]/node.operands[1])
        ...
        for o in node.operands:
            if isinstance(o, ast):
                if o.token == token.identifier:
                    ...
                    l.append([e._asdict()[key]
                              for e in self.e_list[table] if e in entries])
